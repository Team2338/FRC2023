...
public class MotionProfileGenerator {
     ...
     * @return A motion profile from prevState to goalState that satisfies constraints.
     */
    public synchronized static MotionProfile generateProfile(MotionProfileConstraints constraints,
        ...
        double delta_pos = goalState.pos() - prevState.pos();
        ...
        MotionState start_state = new MotionState(prevState.t(), prevState.pos(),
                Math.signum(prevState.vel()) * Math.min(Math.abs(prevState.vel()), constraints.getMaxAbsVel()),
                Math.signum(prevState.acc()) * Math.min(Math.abs(prevState.acc()), constraints.getMaxAbsAcc()));
        ...
        if (start_state.vel() < 0.0 && delta_pos > 0.0) {
            final double stopping_time = Math.abs(start_state.vel() / constraints.getMaxAbsAcc());
            profile.appendControl(constraints.getMaxAbsAcc(), stopping_time);
            start_state = profile.endState();
            delta_pos = goalState.pos() - start_state.pos();
        }
        // Invariant from this point on: start_state.vel() >= 0.0
        final double min_abs_vel_at_goal_sqr = start_state.vel2() - 2.0 * constraints.getMaxAbsAcc() * delta_pos;
        final double min_abs_vel_at_goal = Math.sqrt(Math.abs(min_abs_vel_at_goal_sqr));
        final double max_abs_vel_at_goal = Math.sqrt(start_state.vel2() + 2.0 * constraints.getMaxAbsAcc() * delta_pos);
        double goal_vel = goalState.getMaxAbsVel();
        double max_acc = constraints.getMaxAbsAcc();
        if (min_abs_vel_at_goal_sqr > 0.0
                && min_abs_vel_at_goal > (goalState.getMaxAbsVel() + goalState.getVelTolerance())) {
            // Overshoot is unavoidable with the current constraints. Look at completion_behavior to see what we should
            // do.
            if (goalState.completion_behavior() == MotionProfileGoal.CompletionBehavior.VIOLATE_MAX_ABS_VEL) {
                // Adjust the goal velocity.
                goal_vel = min_abs_vel_at_goal;
            } else if (goalState.completion_behavior() == MotionProfileGoal.CompletionBehavior.VIOLATE_MAX_ACCEL) {
                if (Math.abs(delta_pos) < goalState.getPosTolerance()) {
        ...
        // Invariant from this point forward: We can achieve goal_vel at goalState.pos exactly using no more than +/-
        // max_acc.
        ...
        final double vMax = Math.min(constraints.getMaxAbsVel(),
                Math.sqrt((start_state.vel2() + goal_vel * goal_vel) / 2.0 + delta_pos * max_acc));

        // Accelerate to vMax
        if (vMax > start_state.vel()) {
            final double accel_time = (vMax - start_state.vel()) / max_acc;
        ...
        final double distance_decel = Math.max(0.0,
                (start_state.vel2() - goal_vel * goal_vel) / (2.0 * constraints.getMaxAbsAcc()));
        final double distance_cruise = Math.max(0.0, goalState.pos() - start_state.pos() - distance_decel);

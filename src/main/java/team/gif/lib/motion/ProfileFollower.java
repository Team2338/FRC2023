...
public class ProfileFollower {
    ...
    public MotionState getSetpoint() {
        return (mLatestSetpoint == null ? MotionState.kInvalidState : mLatestSetpoint.motionState);
    }
    ...
    public synchronized double update(MotionState latest_state, double t) {
        ...
        if (mLatestSetpoint != null) {
            prev_state = mLatestSetpoint.motionState;
        } else {
        ...
        // Update error.
        mLatestPosError = mLatestSetpoint.motionState.pos() - latest_state.pos();
        mLatestVelError = mLatestSetpoint.motionState.vel() - latest_state.vel();
        ...
        double output = mKp * mLatestPosError + mKv * mLatestVelError + mKffv * mLatestSetpoint.motionState.vel()
                + (Double.isNaN(mLatestSetpoint.motionState.acc()) ? 0.0 : mKffa * mLatestSetpoint.motionState.acc());
    ...
    public boolean isFinishedProfile() {
        return mGoal != null && mLatestSetpoint != null && mLatestSetpoint.finalSetpoint;
    }
